#ifndef _LANGUAGE_CN_H_
#define _LANGUAGE_CN_H_
#include "../../../../../inc/MarlinConfigPre.h"
#if ENABLED(BTT_UI_SPI)
    #define CN_LANGUAGE             "简体中文"
    #define CN_HEAT                 "加热"
    #define CN_MOVE                 "移动"
    #define CN_HOME                 "回原点"
    #define CN_PRINT                "打印"
    #define CN_EXTRUDE              "挤出"
    #define CN_FAN                  "风扇"
    #define CN_SETTINGS             "设置"
    #define CN_SCREEN_SETTINGS      "屏幕设置"
    #define CN_MACHINE_SETTINGS     "机器设置"
    #define CN_FEATURE_SETTINGS     "功能设置"
    #define CN_SILENT_ON            "静音模式"
    #define CN_SILENT_OFF           "响铃模式"
    #define CN_SHUT_DOWN            "关闭电源"
    #define CN_RGB_SETTINGS         "灯光颜色"
    #define CN_GCODE                "Gcode"
    #define CN_CUSTOM               "自定义"
    #define CN_RGB_RED              "红色"
    #define CN_RGB_GREEN            "绿色"
    #define CN_RGB_BLUE             "蓝色"
    #define CN_RGB_WHITE            "白色"
    #define CN_RGB_OFF              "熄灭灯光"
    #define CN_LEVELING             "调平"
    #define CN_POINT_1              "第一点"
    #define CN_POINT_2              "第二点"
    #define CN_POINT_3              "第三点"
    #define CN_POINT_4              "第四点"
    #define CN_BLTOUCH              "Bltouch"
    #define CN_ABL                  "自动调平"
    #define CN_BLTOUCH_TEST         "BL自检"
    #define CN_BLTOUCH_DEPLOY       "探针弹出"
    #define CN_BLTOUCH_STOW         "探针收回"
    #define CN_BLTOUCH_REPEAT       "精度测试"
    #define CN_PROBE_OFFSET         "Z-Offset"
    #define CN_EEPROM_SAVE          "保存参数"
    #define CN_INC                  "增加"
    #define CN_DEC                  "减少"
    #define CN_NOZZLE               "喷头"
    #define CN_BED                  "热床"
    #define CN_STOP                 "停止"
    #define CN_BACK                 "返回"
    #define CN_PAGE_UP              "上一页"
    #define CN_PAGE_DOWN            "下一页"
    #define CN_PAUSE                "暂停"
    #define CN_RESUME               "继续"
    #define CN_LOAD                 "进料"
    #define CN_UNLOAD               "退料"
    #define CN_SLOW_SPEED           "慢速"
    #define CN_NORMAL_SPEED         "常速"
    #define CN_FAST_SPEED           "快速"
    #define CN_FAN_FULL_SPEED       "全速"
    #define CN_FAN_HALF_SPEED       "半速"
    #define CN_ROTATE_UI            "旋转界面"
    #define CN_TOUCHSCREEN_ADJUST   "触屏校准"
    #define CN_MORE                 "更多"
    #define CN_SCREEN_INFO          "关于"
    #define CN_WHITE                "白色"
    #define CN_BLACK                "黑色"
    #define CN_BLUE                 "蓝色"
    #define CN_RED                  "红色"
    #define CN_GREEN                "绿色"
    #define CN_CYAN                 "青色"
    #define CN_YELLOW               "黄色"
    #define CN_BROWN                "棕色"
    #define CN_GRAY                 "灰色"
    #define CN_DISCONNECT           "断开连接"
    #define CN_BAUDRATE_115200      "115200"
    #define CN_BAUDRATE_250000      "250000"
    #define CN_RUNOUT_OFF           "耗材检测关"
    #define CN_RUNOUT_ON            "耗材检测开"
    #define CN_SMART_RUNOUT_ON      "智能检测开"
    #define CN_PERCENTAGE           "百分比"
    #define CN_BABYSTEP             "Z-微调"
    #define CN_PERCENTAGE_SPEED     "速度百分比"
    #define CN_PERCENTAGE_FLOW      "流量百分比"
    #define CN_VALUE_ZERO           "清零"
    #define CN_1_DEGREE             "1℃"
    #define CN_5_DEGREE             "5℃"
    #define CN_10_DEGREE            "10℃"
    #define CN_X_INC                "X+"
    #define CN_Y_INC                "Y+"
    #define CN_Z_INC                "Z+"
    #define CN_X_DEC                "X-"
    #define CN_Y_DEC                "Y-"
    #define CN_Z_DEC                "Z-"
    #define CN_X_HOME               "X"
    #define CN_Y_HOME               "Y"
    #define CN_Z_HOME               "Z"
    #define CN_001_MM               "0.01mm"
    #define CN_01_MM                "0.1mm"
    #define CN_1_MM                 "1mm"
    #define CN_5_MM                 "5mm"
    #define CN_10_MM                "10mm"
    #define CN_100_MM               "100mm"
    #define CN_1_PERCENT            "1%"
    #define CN_5_PERCENT            "5%"
    #define CN_10_PERCENT           "10%"
    #define CN_READY                "准备打印"
    #define CN_BUSY                 "系统繁忙,请稍候..."
    #define CN_UNCONNECTED          "未连接到打印机!"
    #define CN_DISCONNECT_INFO      "现在可以用电脑控制打印机"
    #define CN_LOADING              "加载中..." 
    #define CN_POWER_FAILED         "是否继续打印?"
    #define CN_CONTINUE             "继续"
    #define CN_CANNEL               "取消"
    #define CN_ADJUST_TITLE         "触屏校准"
    #define CN_ADJUST_INFO          "请点击红点中心位置"
    #define CN_ADJUST_OK            "校准成功"
    #define CN_ADJUST_FAILED        "校准失败,请重试"
    #define CN_WARNING              "警告"
    #define CN_STOP_PRINT           "是否停止打印?"
    #define CN_CONFIRM              "确定"
    #define CN_TFTSD                "TFT SD"
    #define CN_READ_TFTSD_ERROR     "读TFT SD卡出错!"
    #define CN_TFTSD_INSERTED       "SD卡已插入!"
    #define CN_TFTSD_REMOVED        "SD卡已拔出!"    
    #define CN_U_DISK               "U盘"
    #define CN_READ_U_DISK_ERROR    "读U盘出错!"
    #define CN_U_DISK_INSERTED      "U盘已插入!"
    #define CN_U_DISK_REMOVED       "U盘已拔出!"    
    #define CN_ONBOARDSD            "板载SD"
    #define CN_READ_ONBOARDSD_ERROR "读板载SD卡出错!"    
    #define CN_FILAMENT_RUNOUT      "耗材已用尽,请更换耗材!"
    #define CN_PREHEAT              "一键预热"
    #define CN_PREHEAT_BOTH         "Both"
    #define CN_PREHEAT_PLA          "PLA"
    #define CN_PREHEAT_PETG         "PETG"
    #define CN_PREHEAT_ABS          "ABS"
    #define CN_PREHEAT_CUSTOM1      "CUSTOM1"
    #define CN_PREHEAT_CUSTOM2      "CUSTOM2"
    #define CN_IS_PAUSE             "打印中无法挤出耗材, 是否暂停打印?"
    #define CN_AUTO_SHUT_DOWN       "打完关机 开"
    #define CN_MANUAL_SHUT_DOWN     "打完关机 关"
    #define CN_UNIFIEDMOVE          "运动"
    #define CN_UNIFIEDHEAT          "温度"
    #define CN_COOLDOWN             "冷却"
    #define CN_EMERGENCYSTOP        "紧急停止"
    #define CN_TOUCH_TO_EXIT        "触摸任意点退出当前界面"
    #define CN_MAINMENU             "菜单"
    #define CN_WAIT_TEMP_SHUT_DOWN  "喷头温度正在降温，等待低于" STRINGIFY(AUTO_SHUT_DOWN_MAXTEMP) "℃后自动关机" // The nozzle temperature is cooling down, and it will automatically shut down after waiting for it to fall below 50°C
    #define CN_FORCE_SHUT_DOWN      "强制关机"
    #define CN_SHUTTING_DOWN        "正在关机..."
    #define CN_PRINTING_COMPLETED   "打印已完成!"
    #define CN_STATUS_INFO          "状态信息"
    #define CN_ERROR                "严重错误"
    #define CN_TIPS                 "提示"
    #define CN_EEPROM_SAVE_SUCCESS  "EEPROM 参数保存成功"
    #define CN_EEPROM_SAVE_FAILED   "EEPROM 参数保存失败"
    #define CN_SOFTENING            "软化耗材"
    #define CN_DEFAULT              "恢复出厂设置"
    #define CN_IS_DEFAULT           "确认恢复出厂设置？"
    #define CN_DEFAULT_OK           "已恢复出厂设置！"
    #define CN_LEVEL_TEMP           "等待加热棒稳定至140度"
    #define CN_PSTOP                "等待归零结束。。"
    #define CN_LEVEL_BED_TEMP       "热床稳定至60度"
    #define CN_LEVELING_OVEL        "调平完成！"
    #define CN_LEVELING_ING         "调平中。。"
    #define CN_MOVEMENT             "运动"
    #define CN_OFF                  "OFF"
    #define CN_ON                   "ON"
    #define CN_COLDEXTRUD           "禁止冷挤出（T0<170度）"
    #define CN_COLDEXTRUD170        "预热"
    #define CN_CLEARNOZZ            "请清理喷头！"
    #define CN_MODE_SELECT          "模式选择"
    #define CN_POWERLOSS_ON         "断电续打开"
    #define CN_POWERLOSS_OFF        "断电续打关"
    #define CN_MOTOR_UNLOCK         "电机解锁"
    #define CN_MESH_VALID           "调平测试"
    #define CN_WAIT_FOR_TEMP        "请等待温度升高到目标温度"
    #define CN_STOP_PLANE_TEST      "是否停止调平测试？"
    #define CN_IS_BABYSTEP          "确认保存微调值？"
    #define CN_IS_LANGUAGE          "确定保存设置？"
    #define CN_HOT_HOMING_ON        "热归零：开"
    #define CN_HOT_HOMING_OFF       "热归零：关"
    #define CN_HOT_HOMING_START     "热归零：启动！"
    #define CN_HOT_HOMING_Z_HOMING  "热归零：Z轴归零中！"    
    #define CN_HOT_HOMING_STOP      "热归零：完成！"  
    #define CN_LIST_MODE            "列表模式"
    #define CN_ICON_MODE            "图标模式"
#endif
#endif

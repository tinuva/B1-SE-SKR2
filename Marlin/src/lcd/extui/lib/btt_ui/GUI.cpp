...
void PRINT_GUI_DrawPixel(int16_t x, int16_t y, uint16_t color)
{	   
  LCD_SetWindow(x, y, x, y);

  #if LCD_HAL == LCD_HAL_STM32F407
   PRINT_SPI_TFT_Write16Data(color);	
  ...
  uint8_t temp,t1,t;
	...
	{   
	  temp=asc2_1608[num][t];				
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)PRINT_GUI_DrawPixel(x,y,foreGroundColor);
			else if(mode==0)PRINT_GUI_DrawPixel(x,y,backGroundColor);
			temp<<=1;
			y++;
			if(y>=LCD_HEIGHT_PIXEL)return;		
			if((y-y0)==16)
			{
				y=y0;
				x++;
				if(x>=LCD_WIDTH_PIXEL)return;	
				break;
			}
		}  	 
	}  	    	   	 	  
...
{  
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode&0X80)LCD_ShowChar(x+8*t,y,'0',mode&0X01);  
				else LCD_ShowChar(x+8*t,y,' ',mode&0X01);  
 				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+8*t,y,temp+'0',mode&0X01); 
	}
} 
...
{    
  ...
  floatBuf[bufIndex] = 0;
  LCD_ShowString(x,y,sizeof(floatBuf),16,floatBuf);
  //GUI_DispString(x, y, floatBuf);
} 
...
{         	
  ...
  decBuf[bufIndex] = 0;
  LCD_ShowString(x,y,sizeof(decBuf),16,decBuf);
//  GUI_DispString(x, y, decBuf);
} 

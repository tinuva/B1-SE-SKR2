...
#ifdef Z_HOT_HOMING
void z_hot_homing(void)
{
    static int16_t save_temp=thermalManager.degTargetHotend(0);
    if(TERN_(EXTENSIBLE_UI, ExtUI::onIshothoming()))
    {
      TERN_(EXTENSIBLE_UI, ExtUI::onZHotHomingStart());
    }
    while(TERN_(EXTENSIBLE_UI, ExtUI::onIshothoming()))
    {
      if(thermalManager.degTargetHotend(0)!=HOT_HOMING_MAX_TEMP)
      {
        thermalManager.setTargetHotend(HOT_HOMING_MAX_TEMP,0);
      }
      idle();
    }
    if(TERN_(EXTENSIBLE_UI, ExtUI::onIsprintfing()))
    {
     TERN_(EXTENSIBLE_UI, ExtUI::onZHotHomingRuning());
     thermalManager.setTargetHotend(save_temp,0);
    }
}
#endif
  ...
  inline void home_z_safely() {
    ...
    if (position_is_reachable(destination)) {
      ...
      do_blocking_move_to_xy(destination);
       ...
       #ifdef Z_HOT_HOMING
         TERN_(Z_HOT_HOMING, z_hot_homing());
       #endif  
       #ifdef FIX_MOUNTED_PROBE
         WRITE(PROVE_CONTROL_PIN, HIGH);
       #endif
      homeaxis(Z_AXIS);
      #ifdef Z_HOT_HOMING
         TERN_(EXTENSIBLE_UI, ExtUI::onZHotHomingStop());
      #endif 
    }
    else {
